...
export default function UsersPage() {
  ...
  const fetchUsers = async (isManual = false) => {
    try {
      if (isManual) {
        setRefreshing(true);   // refresh button spinner
      } else {
        setLoading(true);      // table loading
      }

      const res = await instance.get(
        "/api/admin/userAnalytics/users/dashboard",
        {
          params: {
            page: currentPage,
            limit: usersPerPage,
  ...
  useEffect(() => {
    fetchUsers(false);
  }, [currentPage, statusFilter, planFilter, debouncedSearch, selectedDate]);

  /* ---------------- Pagination Calculation ---------------- */
  const totalPages = useMemo(() => {
    return Math.ceil(stats.totalUsers / usersPerPage);
  }, [stats.totalUsers]);
  ...
  /* ---------------- CSV Export ---------------- */
  const handleExport = () => {
    const headers = ["Name", "Email", "Subscription", "Status"];

    const rows = users.map((u) =>
      [u.name, u.email, u.subscription, u.status].map(
        (field) => `"${field ?? ""}"`
      )
    );
  ...
  const StatCard = ({ title, value }: any) => (
    <div className="bg-[#131313] border border-[#1F1F1F] rounded-xl p-5">
  ...
  return (
    <div className="bg-black min-h-screen text-white mt-[5%]">
      <div className="w-full max-w-[1800px] mx-auto px-2 py-6 space-y-6 ">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <StatCard title="Suspended" value={stats.suspendedUsers} />
        </div>
        ...
        <div className="flex gap-3">
          <div className="relative">
            <button
              ...
              className="bg-[#131313] border border-[#1F1F1F] rounded-lg px-4 py-2 text-sm"
            >
              {selectedDate
            ...
            {showCalendar && (
              <div className="absolute z-50 mt-2 bg-[#131313] border border-[#1F1F1F] rounded-xl p-3">
                <DayPicker
                  mode="single"
          ...
          <button
            onClick={() => fetchUsers(true)}   // manual call
        ...
        {/* SEARCH + FILTER */}
        <div className="flex justify-between bg-[#131313] border border-[#1F1F1F] rounded-xl p-4">
          <div className="relative w-[700px]">
            ...
            <input
              ...
              placeholder="Search by name, email or user ID "
              className="w-full h-[42px] bg-transparent border border-[#1F1F1F] rounded-lg pl-10 text-sm outline-none"
            />
          </div>
          ...
          <div className="flex gap-3">
            <select
              ...
              className="bg-[#131313] border border-[#1F1F1F] rounded-lg px-4 h-[42px]"
            >
              <option>All status</option>
              <option>ACTIVE</option>
              <option>INACTIVE</option>
            ...
            <button
              ...
              className="px-5 rounded-lg flex items-center gap-2 text-white"
              style={{
                background:
        ...
        {/* TABLE + PAGINATION */}
        <div className="bg-[#131313] border border-[#1F1F1F] rounded-xl min-h-[520px] flex flex-col justify-between">
{loading || refreshing ? (
                ...
                <tbody>
                  {users.map((u, i) => (
                    <tr key={i} className="border-b border-[#1F1F1F]">
                      ...
                      <td className="p-4">{u.subscription}</td>
                      <td className="p-4">
                        <span
                          className={`px-3 py-1 rounded-full text-xs ${u.status === "ACTIVE"
                              ? "bg-emerald-500/10 text-emerald-400"
                              : u.status === "INACTIVE"
                                ? "bg-yellow-500/10 text-yellow-400"
                                : "bg-red-500/10 text-red-400"
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (pageGroup > 0) {
                          const newGroup = pageGroup - 1;
                          setPageGroup(newGroup);
                          setCurrentPage(newGroup * pagesPerGroup + 3);
                        }
                      }}
                      className="px-4 py-2 border border-[#1F1F1F] bg-[#131313] disabled:opacity-40"
                    >
                      Previous
                    </button>
