...
import { FiRefreshCcw, FiPlus, FiCalendar, FiChevronDown, FiSettings, FiEdit } from "react-icons/fi";
import { DayPicker } from "react-day-picker";
import "react-day-picker/dist/style.css";
...
const Creditsplans: React.FC = () => {
  ...
  const fetchDashboard = async () => {
    try {
      setLoading(true);
      const res = await instance.get("/admin/dashboard-data");
        ...
        <div className="flex gap-3">
          ...
          <div className="relative">
            ...
            >
              <FiCalendar className="text-white text-lg" />
              <span className="text-white font-medium">
                {selectedDate ? selectedDate.toLocaleDateString() : "Last 30 days"}
              </span>
              <FiChevronDown className="text-white text-lg ml-2" />
            </button>
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            bg-gradient-to-r from-orange-400 via-pink-500 to-purple-600">
              ...
              <button
                ...
                text-lg font-medium border border-[#1F1F1F]"
              >
                <FiPlus className="text-xl" />
                Add New Plan
              </button>
  ...
  Create Plan
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {plans.map((plan) => (
              ...
              >
                <div className={`bg-gradient-to-r ${plan.gradient} p-6`}>
                  <h3 className="text-sm font-medium opacity-90">{plan.name}</h3>

                  <div className="mt-3 flex items-end gap-1">
                    <span className="text-3xl font-bold">{plan.credits}</span>
                    <span className="text-sm opacity-80 mb-1">Credits</span>
                  </div>
